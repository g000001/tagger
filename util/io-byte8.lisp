...
#-(or xerox cmu) `(file-position ,stream))
...
#-(or xerox cmu) `(file-position ,stream ,position))
 
(defmacro byte8-stream-length (stream)
  `(progn (break "byte8-stream-length !") (file-length ,stream)))
...
#-(or xerox lucid cmu) `(write-byte ,byte8 ,stream))
...
(defmacro byte8-read (stream)
  `(the byte8
	...
	#-(or xerox lucid cmu mcl) (read-byte (the stream ,stream))))
...
(declaim (ftype (function (stream) byte16) byte16-read))
